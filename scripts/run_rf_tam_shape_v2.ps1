param(
    ...
    [int]$ParetoSamples = 8192,
    [string]$RefineKeepRatios = "1.0,0.95,0.90,0.85",
    [int]$Stage3FixedProbeSamples = 4096,
    [int]$Stage3FixedProbeTrainSamples = 30000,
    [int]$Stage3FixedProbeValSamples = 5000,
    [int]$Stage3FixedProbeEpochs = 8,
    [double]$DirectionCorrectionStrength = 0.85,
    [double]$MaxIncomingDummyShare = 0.20,
    [double]$TamFlattenStrength = 0.45,
    [double]$TamFlattenFloor = 1.0,
    [int]$LogEvery = 250,
    [switch]$SkipDefense,
...
if (-not $RunName.Trim()) {
    ...
    $RunName = "dmmpv3_rf_tam_shape_v2_fullcw_seed${Seed}_${stamp}"
}
...
$TranscriptPath = Join-Path $LogRoot "${RunName}_rf_tam_shape_v2_${LogStamp}.log"
...
$defenseCmd = @(
    ...
    "--tam_flatten_strength", ([string]$TamFlattenStrength),
    "--tam_flatten_floor", ([string]$TamFlattenFloor),
    ...
    "--stage3_fixed_probe_weight", "2.00",
    "--stage3_fixed_probe_min_clean_accuracy", "0.80",
    ...
    "--stage3_min_dummy_incoming_share", "0.0",
    "--stage3_max_dummy_incoming_share", ([string]$MaxIncomingDummyShare),
    "--stage3_min_tam_incoming_l1_shift", "0.0",
    "--stage3_incoming_metric_weight", "0.50",
...
$manifest = [ordered]@{
    run_name = $RunName
    run_dir = $RunDir
    method = "dmmpv3_rf_tam_shape_v2"
    budget = $Budget
    full_cw = $true
    rf_only_attack = $true
    natural_direction_target = $true
    max_incoming_dummy_share = $MaxIncomingDummyShare
    tam_flatten_strength = $TamFlattenStrength
    tam_flatten_floor = $TamFlattenFloor
...
$manifestPath = Join-Path $OutputDir "${RunName}_rf_tam_shape_v2_invocation.json"
...
try {
    ...
    if (-not $SkipDefense) {
        Invoke-ExperimentStep -Name "Defense Stage $DefenseStage RF TAM-shape natural-direction diffusion" -Command $defenseCmd
    }
    ...
    if (-not $DryRun -and (Test-Path $RunDir)) {
        Set-Content -Path (Join-Path $RunDir "rf_tam_shape_v2_invocation.json") -Value $manifestJson -Encoding UTF8
    }
} finally {
...
Write-Host ""
Write-Host "RF TAM-shape natural-direction experiment run_dir: $RunDir"
Write-Host "Realtime transcript: $TranscriptPath"
