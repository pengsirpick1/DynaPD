# -*- coding: utf-8 -*-
"""Run E2b with an optional stochastic completion stage.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--completion_mode", choices=["off", "preserve_flip_fill"], default="off")
    parser.add_argument("--completion_target_bandwidth", type=float, default=0.0)
    parser.add_argument("--completion_topk", type=int, default=16)
...
def _candidate_margins(candidate_probs: dict[str, np.ndarray], idx: int, ref_classes: dict[str, int]) -> dict[str, float]:
    return {
        name: _reference_margin(np.asarray(candidate_probs[name][idx], dtype=np.float32), int(ref_classes[name]))
        for name in MODEL_NAMES
    }


def _choose_completion_action(
    *,
    method: str,
    actions: list[Any],
    ...
    if str(args.completion_mode) != "preserve_flip_fill" or not actions:
        return -1
    valid: list[tuple[float, float, int]] = []
    for idx, action in enumerate(actions):
        margins = _candidate_margins(candidate_probs, idx, ref_classes)
        if not _target_reached(str(method), margins, float(args.margin_target)):
            continue
        cost = float(action_cost(action, clean_total))
        valid.append((cost, float(scores[idx]) if len(scores) else 0.0, int(idx)))
    if not valid:
        return -1
    valid.sort(key=lambda item: (item[0], item[1]), reverse=True)
    topk = max(1, min(int(args.completion_topk), len(valid)))
    return int(np.random.choice([idx for _cost, _score, idx in valid[:topk]]))
...
def _run_ensemble_controller(
    *,
    ...
    completion_target = float(getattr(args, "completion_target_bandwidth", 0.0) or 0.0)
    if str(getattr(args, "completion_mode", "off")) != "off" and completion_target <= 0.0:
        completion_target = float(budget)
    completion_target = min(float(budget), max(0.0, completion_target))
    ...
    for round_index in range(max(1, int(args.rounds))):
        ...
        target_now = _target_reached(str(method), _model_margins(current_probs, ref_classes), float(args.margin_target))
        completion_active = (
            str(getattr(args, "completion_mode", "off")) != "off"
            and target_now
            and float(current.dummy_bandwidth) < float(completion_target)
        )
        if target_now and not completion_active:
        ...
        for step_index in range(int(args.max_dummy_steps)):
            ...
            if completion_active:
                selected_idx = _choose_completion_action(
                    method=str(method),
                    actions=actions,
                    candidate_probs=_candidate_probs,
                    scores=scores,
                    ref_classes=ref_classes,
                    clean_total=clean_total,
                    args=args,
            ...
            if selected_idx < 0:
                if completion_active:
                    stop_reason = "completion_no_preserve_flip"
                else:
        ...
        if stop_reason in {"target_reached", "all_models_target_reached", "bandwidth_10pct_reached", "candidate_pool_exhausted", "no_positive_single", "no_robust_positive", "completion_no_preserve_flip"}:
            break
...
def main() -> None:
    ...
    manifest = {
        ...
        "completion_target_bandwidth": float(args.completion_target_bandwidth),
