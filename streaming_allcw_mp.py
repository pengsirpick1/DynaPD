"""Full all-CW evaluation for audited streaming DynaPD-RT variants.

Dataset:
  datasets/CW.npz, 105730 traces, original length 10000.

Evaluation protocol:
  - truncate every trace to the first 5000 entries, matching existing CW
    attack checkpoints and RT scripts.
  - generation is CPU multiprocessing, chunked to avoid loading all X at once.
  - evaluation is single-process GPU batched over saved chunks.
"""
...
VARIANTS = ("streaming_tail1", "streaming_tail0", "batch_fullinfo")
OUT_DIR = ROOT / "results/night_batch_20260805/streaming_allcw"
...
def _extract_pos_bursts(trace: np.ndarray) -> list[tuple[int, int, int]]:
    nz = np.flatnonzero(trace > 0)
    if len(nz) == 0:
        return []
    times = np.abs(trace[nz])
    slots = np.floor(times * (1799.0 / 80.0)).astype(int)
    slots = np.clip(slots, 0, sm.BINS - 1)
    slots = np.sort(slots)
    ...
    for slot in slots[1:]:
        s = int(slot)
        if s - cur_e <= sm.GAP_THRESH:
            cur_e = s
            cur_n += 1
        else:
            bursts.append((cur_s, cur_e, cur_n))
            cur_s = cur_e = s
            cur_n = 1
    bursts.append((cur_s, cur_e, cur_n))
    return bursts
...
def defend_batch_fullinfo(trace: np.ndarray, seed: int) -> tuple[np.ndarray, dict]:
    clean_total = float((trace != 0).sum())
    budget = int(clean_total * 0.25)
    bursts = _extract_pos_bursts(trace)
    counts = np.zeros((2, sm.BINS), dtype=np.int32)
    used = 0
    delay_windows: list[tuple[int, int, int]] = []

    if bursts:
        per_b = max(1, int(budget / len(bursts)))
        for bi, (_start, end, _n) in enumerate(bursts):
            if used >= budget:
                break
            if bi < len(bursts) / 3:
                phase = "early"
            elif bi < 2 * len(bursts) / 3:
                phase = "mid"
            else:
                phase = "late"
            dose = sm.best_dose(phase, per_b)
            dose = min(max(dose, int(per_b * 0.7)), budget - used)
            for bin_idx in range(end, min(sm.BINS, end + dose)):
                if used >= budget:
                    break
                counts[0, bin_idx] += 1
                used += 1
            delay_windows.append((int(end), max(0, int(end) - sm.DELAY_WIN), min(sm.BINS, int(end) + 16)))

    defended, _tam, stats = sm._render_dummy(base_trace=trace, counts=counts, args=sm._make_args(seed))

    if delay_windows:
        rng = np.random.default_rng(seed)
        nz2 = np.flatnonzero(defended != 0)
        if len(nz2):
            times = np.abs(defended[nz2])
            signs = np.sign(defended[nz2])
            slots = np.floor(times * (1799.0 / 80.0)).astype(int)
            slots = np.clip(slots, 0, sm.BINS - 1)
            for _decision_bin, win_start, win_end in delay_windows:
                in_win = (slots >= win_start) & (slots < win_end)
                if in_win.any():
                    d_bins = rng.integers(1, sm.MAX_DELAY + 1, size=int(in_win.sum()))
                    times[in_win] += d_bins * (80.0 / 1800.0)
            defended[nz2] = signs * times
            order = np.argsort(np.abs(defended))
            defended = np.pad(defended[order], (0, max(0, sm.SL - len(defended))), mode="constant")[: sm.SL]
...
def _worker_one(args: tuple[str, int, np.ndarray]) -> tuple[int, np.ndarray, dict]:
    variant, idx, trace = args
    trace = np.asarray(trace, dtype=np.float32)[: sm.SL]
    sm.USE_DELAY = True
    if variant == "streaming_tail1":
        sm.TAIL_ACTION = True
        defended, dbg = sm.defend_stream(trace, idx, debug=True)
        meta = {
    ...
    elif variant == "batch_fullinfo":
        defended, meta = defend_batch_fullinfo(trace, idx)
    else:
...
def _meta_summary(items: list[dict], variant: str, n: int, workers: int, chunk_size: int, elapsed: float) -> dict:
...
def generate_variant(
    ...
    for chunk_id, start in enumerate(range(0, n, chunk_size)):
        ...
        tasks = [(variant, start + j, X_chunk[j]) for j in range(len(X_chunk))]
    ...
    summary = _meta_summary(all_meta, variant, n, workers, chunk_size, time.time() - t0)
...
def main() -> None:
    ...
    print(f"[MAIN] ALL-CW: X={X_src.shape}, y={y_src.shape}, variants={variants}", flush=True)
    ...
    for variant in variants:
        if variant not in VARIANTS:
            raise ValueError(f"unknown variant: {variant}")
        manifest["generation"][variant] = generate_variant(variant, X_src, y_src, args.workers, args.chunk_size)
    ...
    print("[TABLE] variant,WC,RF,DF,TF,AWF,VarCNN,BW,gen_ms,audit_future", flush=True)
    for variant in variants:
        ...
        print(
            "[TABLE] "
            f"{variant},{100*ev['wc']:.2f},{100*acc['RF']:.2f},{100*acc['DF']:.2f},"
